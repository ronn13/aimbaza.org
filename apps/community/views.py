...
def _load_partners():
    ...
    for p in entries:
        ...
        label, icon, colour = _TYPE_META.get(t, ("Partner", "fas fa-handshake", "secondary"))
        p["type_label"] = label
        p["type_icon"] = icon
...
def _load_calendar(partners):
    ...
    for e in entries:
        if not e.get("active", True):
            continue
        t = e.get("type", "event")
        label, icon, colour = _CAL_TYPE_META.get(t, ("Event", "fas fa-calendar", "secondary"))
        e["type_label"] = label
        e["type_icon"] = icon
        e["type_colour"] = colour
        e["partners"] = [partners[pid] for pid in e.get("partner_ids", []) if pid in partners]
        result.append(e)
...
def partners(request):
    ...
    return render(request, "community/partners.html", {
